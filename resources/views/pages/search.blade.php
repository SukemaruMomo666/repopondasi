...
<head>
    ...
    <style>
        ...
        /* CHECKBOX FILTER JENIS MITRA */
        .filter-item-label { display: flex; align-items: center; gap: 10px; cursor: pointer; color: #475569; font-size: 0.95rem; padding: 8px 10px; border-radius: 6px; transition: 0.2s; }
        .filter-item-label:hover { background: #f8fafc; color: #0f172a; }
        .custom-cb { accent-color: #2563eb; width: 16px; height: 16px; cursor: pointer; }
        ...
        .product-image { width: 100%; aspect-ratio: 1/1; background: #f8fafc; position: relative;}
        ...
        .view-more-btn {
            background: transparent; border: none; color: #2563eb; font-size: 0.9rem; font-weight: 600; padding: 10px; width: 100%; text-align: left; cursor: pointer; display: flex; justify-content: space-between; align-items: center; border-radius: 6px; margin-top: 5px; transition: all 0.2s ease;
        }
        .view-more-btn:hover { background-color: #eff6ff; color: #1e3a8a; }
...
<body class="text-zinc-800 antialiased pt-[80px]">
    ...
    <div class="search-container">
       ...
       {{-- SIDEBAR KIRI: FILTER KATEGORI & JENIS MITRA --}}
        <aside class="filter-sidebar">
            ...
            <ul class="cat-list">
                {{-- Tombol Semua Kategori (Reset Filter Kategori Saja, tetap bawa filter lain) --}}
                <li>
                    <a href="{{ request()->fullUrlWithQuery(['kategori' => null]) }}" class="cat-link {{ empty($kategoriId) ? 'active' : '' }}">
                        Semua Kategori
                    </a>
                </li>
            ...
            {{-- FORM FILTER LAINNYA (CHECKBOX) --}}
            <form action="{{ url()->current() }}" method="GET" id="filterForm" class="mt-8 pt-6 border-t border-slate-100">
                {{-- Hidden input agar query & kategori tidak hilang saat form di-submit --}}
                @if(request('query')) <input type="hidden" name="query" value="{{ request('query') }}"> @endif
                @if(request('kategori')) <input type="hidden" name="kategori" value="{{ request('kategori') }}"> @endif

                <h2 class="filter-title mb-3" style="border-bottom:none; padding-bottom:0;"><i class="fas fa-store-alt"></i> Jenis Mitra</h2>
                <div class="flex flex-col gap-2">
                    <label class="filter-item-label">
                        <input type="checkbox" name="tier_toko[]" value="official_store" class="custom-cb" onchange="document.getElementById('filterForm').submit()" {{ in_array('official_store', request('tier_toko', [])) ? 'checked' : '' }}>
                        <span><i class="fas fa-crown text-purple-500 w-4 text-center"></i> Official Store</span>
                    </label>
                    <label class="filter-item-label">
                        <input type="checkbox" name="tier_toko[]" value="pro_merchant" class="custom-cb" onchange="document.getElementById('filterForm').submit()" {{ in_array('pro_merchant', request('tier_toko', [])) ? 'checked' : '' }}>
                        <span><i class="fas fa-check-circle text-emerald-500 w-4 text-center"></i> Pro Merchant</span>
                    </label>
                </div>
            </form>
        </aside>
                                ...
                                <div class="product-details">
                                    ...
                                    <div class="product-footer">
                                        <div class="product-location">
                                            <i class="fas fa-map-marker-alt" style="color: #94a3b8;"></i> <span class="truncate">{{ $p->kota_toko ?? 'Indonesia' }}</span>
                                        </div>
                                        <div class="product-location" style="color: #10b981; font-weight: 600;">
                                            <i class="fas fa-store"></i> <span class="truncate max-w-[120px]">{{ $p->nama_toko }}</span>

                                            {{-- BADGE OFFICIAL / PRO --}}
                                            @if(isset($p->tier_toko) && $p->tier_toko == 'official_store')
                                                <span class="badge-store badge-official" title="Official Store"><i class="fas fa-crown"></i></span>
                                            @elseif(isset($p->tier_toko) && $p->tier_toko == 'pro_merchant')
